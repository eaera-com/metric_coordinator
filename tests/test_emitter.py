...
def test_clickhouse_data_emitter_emit(setup_and_teardown_clickhouse_emitter):
    ...
    df_history = pd.DataFrame(
        [MT5DealDaily(Login=1999, timestamp_utc=int(datetime.datetime.now().timestamp())).model_dump()],
        columns=MT5DealDaily.model_fields.keys(),
    )  # get default value
    df_deal = pd.DataFrame(
        [MT5Deal(login=1999, Deal=1502).model_dump(), MT5Deal(login=1999, Deal=2708).model_dump()], columns=MT5Deal.model_fields.keys()
    )  # get default value
    data = {MT5Deal: df_deal, MT5DealDaily: df_history}
...
def test_clickhouse_data_emitter_emit_duplicate_key(setup_and_teardown_clickhouse_emitter):
    ...
    df_deal = pd.DataFrame(
        [MT5Deal(login=1999, Deal=1502).model_dump(), MT5Deal(login=1999, Deal=1502).model_dump()], columns=MT5Deal.model_fields.keys()
    )  # get default value
    data = {
        MT5Deal: df_deal,
    }
