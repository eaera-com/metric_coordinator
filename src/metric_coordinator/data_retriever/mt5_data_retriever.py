...
class MT5DataRetriever(DataRetriever):
    ...
    def retrieve_data(self,from_time: Annotated[int, "timestamp time-server"],
        ...
        if filters["group_by"] == "Login":
            ...
            self.last_retrieve_timestamp = to_time + self.mt5_manager.TimeGet().TimeZone * 60 + 3600 * (self.mt5_manager.TimeGet().DaylightState)
        return data
    ...
    def retrieve_deals_by_logins(
        ...
        if deals is False:
            return pd.DataFrame(columns=["Login", "Time", "PositionID"]), pd.DataFrame(columns=["Login", "Datetime", "Balance", "ProfitEquity"])
