...
class LocalDatastore(Datastore):
    ...
    def __init__(self, metric: MetricData, sharding_columns: tuple[str] = None, batch_size: int = 64) -> None:
        self.metric = metric
        self.sharding_columns = sorted(
            sharding_columns) if sharding_columns else None
    ...
    def _extract_shard_key_values(self, shard_key: Dict[str, Any]) -> tuple:
        ...
        missing_columns = [
            col for col in self.sharding_columns if col not in shard_key]
        if missing_columns:
            raise ValueError(f"Missing columns in shard_key: {
    ...
    def get_latest_row(self, shard_key: Dict[str, Any]) -> pd.Series:
        ...
        result_df = self.shard_key_values_to_dataframe.get(
            self._extract_shard_key_values(shard_key), None)
        if result_df is None:
            return pd.Series(self.metric(**shard_key).model_dump())
        return result_df.iloc[-1]

    def get_row_by_timestamp(self, shard_key: Dict[str, Any], timestamp: datetime.date, timestamp_column: str, use_default_value: bool = False) -> pd.Series:
        if self.metric is None:
            raise ValueError("Datastore is not initialized or deactivated")
        result_df = self.shard_key_values_to_dataframe.get(
            self._extract_shard_key_values(shard_key), None)
        ...
        if timestamp_column not in shard_key:
            ...
            assert shard_key[timestamp_column] == timestamp, f"Timestamp {
                shard_key[timestamp_column]} != {timestamp}"

        # TODO: optimize for subset of shard_key extraction
        final_result_df = self.subset_df(result_df, shard_key)
        ...
        if value.shape[0] >= self.MINIMUM_VALUE_PUT_IN_BATCH and self.BATCH_PROCESSING:
            ...
            for _, row in value.iterrows():
                ...
                self.shard_key_values_to_dataframe[shard_key_values] = pd.concat([
                    self.shard_key_values_to_dataframe[shard_key_values],
    ...
    def _put_in_batch(self, value):
        ...
        for _, row in value.iterrows():
            ...
            if len(batch[self._extract_shard_key_values(key_values)]) == self._batch_size:
                self.shard_key_values_to_dataframe[self._extract_shard_key_values(key_values)] = \
                    pd.concat(batch[self._extract_shard_key_values(
                        key_values)], ignore_index=True)
                batch[self._extract_shard_key_values(key_values)] = []
        # flush remaining batch
        for key, rows in batch.items():
            self.shard_key_values_to_dataframe[key] = pd.concat(
