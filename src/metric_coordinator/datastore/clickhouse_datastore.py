...
class ClickhouseDatastore(BaseDatastore):
    ...
    def get_metric_fields(self, shard_key: Dict[str, Any]) -> str:
        if len(shard_key) == 0:
            return ""
            ...
            return " AND ".join([f"{k} = {self.get_value_field(v)}" for k, v in shard_key.items()])
    ...
    def get_latest_row(self, shard_key: Dict[str, int]) -> pd.Series:
        ...
        query = f"""
        SELECT * FROM {self.get_metric_table_name()} FINAL WHERE {self.get_metric_fields(shard_key)}
    ...
    def get_row_by_timestamp(
        ...
        if self.metric is None:
            raise ValueError("Datastore is not initialized or deactivated")
        query = f"""
        SELECT * FROM {self.get_metric_table_name()} FINAL WHERE {self.get_metric_fields(shard_key)} AND {timestamp_column} = {self.get_value_field(timestamp)}
        ORDER BY timestamp_server DESC
        LIMIT 1
        """
        result = self.client.query_df(query)
